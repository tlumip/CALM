    ...
    # Martin Mann 05/23/16  In the createExtMats function: Added code to read in a file to move some external trips to a different external station
    # Martin Mann 06/16/16  Added the removal of ii trips from the array ext.ZnZnTdMd before exporting in the function "createExtMats".  Also moved writing out "selectLinks_Report.csv" to after adding
    #                       the period volumes               
    # Alex Bettinardi     alexander.o.bettinardi@odot.state.or.us 7/20/16- fixed identifed time-of-day coding issue, and corrected issue where county employment projections are assumed to hold true into the future.  
    ...
    fun$procSLDataSets <- function(SWIM_SL_Filename_Pattern, inputLoc) {
      ...
      fun$createExtMats <- function( disaggregateMethod, inputLoc, storeLoc) {
                            ...
                            for(d in 1:length(datasets)) {
                                  ...
                                  #Update County Population controls
                                  ...
                                  #=================================================================================================
                                  # If required, adjust some destinations and origins to a different external station
                                  #=================================================================================================

                                  if(file.exists("inputs/externalModel/extStaAdjDF.RData")){
                                       load("inputs/externalModel/extStaAdjDF.RData")
                                       for(i in 1:nrow(extStaAdjDF)) {
                                            data[extStaAdjDF$EZD[[i]],"EXTERNAL_ZONE_DESTINATION"] <- extStaAdjDF$EXTERNAL_ZONE_DESTINATION[i]
                                            data[extStaAdjDF$OZD[[i]],"EXTERNAL_ZONE_ORIGIN"] <-  extStaAdjDF$EXTERNAL_ZONE_ORIGIN[i]
                                       }
                                  }
                                  ...
                                  delete <- sapply(c("ext.ZnZnTdMd","LocalZones","Crosswalk"),function(x)assign(x, get(x), envir = e1))
                            ...
                            #Remove ii Trips  #Added by MM 06/16/16
                            index <- expand.grid(dimnames(ext.ZnZnTdMd)[[3]],dimnames(ext.ZnZnTdMd)[[4]])
                            rcIndx <- (length(externalZones)+1):nrow(ext.ZnZnTdMd)
                            for(x in 1:nrow(index))  ext.ZnZnTdMd[rcIndx,rcIndx,index[x,1],index[x,2]] <- 0
    ...
    fun$ipfExtMatsToCounts <- function(IPF,storeLoc) {
                                ...
                                if(exists("osumFun")) write.csv(externals, paste(storeLoc, "selectLinks_Report.csv", sep=""), row.names=F)
                                
                                #create period volume controls #Moved before writing out "selectLinks_Report.csv"
